...
%ASWconfig set:
setActiveConfigSet(mdlNames.ASW_Structure,'Ref_ConfigSet_PIL_ASW_F28379D');

for k=1:numel(mdlNames.SoftwareModule)
    load_system(mdlNames.SoftwareModule(k).Name);
    setActiveConfigSet(mdlNames.SoftwareModule(k).Name,'Ref_ConfigSet_PIL_ASW_F28379D');
end
